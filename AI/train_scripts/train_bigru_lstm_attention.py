...
# ==============================
# 3) BUILD BiGRU + LSTM + ATTENTION MODEL
# ==============================
...
def train():
    ...
    model = build_bigru_lstm_attention(X_train.shape[1], X_train.shape[2])

    callbacks = [
        EarlyStopping(monitor='val_loss', patience=8, restore_best_weights=True),
        ReduceLROnPlateau(monitor='val_loss', factor=0.4, patience=4),
        ModelCheckpoint("../models/BEST_BiGRU_Attention_LSTM.keras", save_best_only=True, monitor='val_loss')
    ]
    ...
    model.save("../models/FINAL_BiGRU_Attention_LSTM.keras")
